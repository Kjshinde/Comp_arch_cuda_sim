...
class CUDANeuralNetwork:
    ...
    def _load_linear_library(self):
        ...
        # Set argument + return types for GPU memcpy 
        ...
        #function signature for linear forward kernel
        lib.linear_forward_wrapper.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,  # input, W, b, output
            ctypes.c_int, ctypes.c_int, ctypes.c_int                              # batch, in_dim, out_dim
        ]
        lib.linear_forward_wrapper.restype = None

        #function signature for linear backward kernel
        lib.linear_backward_wrapper.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,                   # grad_out, input, W
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,                   # grad_in, grad_W, grad_b
            ctypes.c_int, ctypes.c_int, ctypes.c_int                             # batch, in_dim, out_dim
        ]
    ...
    def _load_relu_library(self):
        ...
        #function signature for ReLU forward kernel
        # input_ptr, output_ptr, num_elements
        lib.relu_forward_wrapper.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int]
        lib.relu_forward_wrapper.restype = None  # returns nothing

        #function signature for ReLU backward kernel
        # grad_out_ptr, input_ptr, grad_in_ptr, num_elements
        lib.relu_backward_wrapper.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int]
        lib.relu_backward_wrapper.restype = None  # returns nothing
        return lib
    ...
    def forward(self, X):
        ...
        # Forward pass through first linear layer
        self.linear_lib.linear_forward_wrapper(
            self.gpu_input,    # input batch for GPU
            self.gpu_W1,       # layer 1 weights
            self.gpu_b1,       # layer 1 bias
            self.gpu_hidden,   # output buffer for hidden activations
            batch_size,        # number of samples
            self.input_size,   # input feature size
            self.hidden_size   # number of hidden neurons
        )
        ...
        # Forward pass through second linear layer
        self.linear_lib.linear_forward_wrapper(
            self.gpu_hidden,   # hidden layer output
            self.gpu_W2,       # layer 2 weights
            self.gpu_b2,       # layer 2 bias
            self.gpu_output,   # final output buffer
            batch_size,        # batch size
            self.hidden_size,  # number of neurons in the hidden layer
            self.output_size   # output classes
        )
    ...
    def backward(self, X, y, output):
        ...
        self.relu_lib.relu_backward_wrapper(
            self.gpu_grad_hidden,   # gradient from the next layer
            self.gpu_hidden,        # stored pre-ReLU activations
            self.gpu_grad_hidden,   # final gradient written back here
            batch_size * self.hidden_size  # total elements
        )
