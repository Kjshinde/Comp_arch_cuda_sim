...
torch::Tensor relu_backward(torch::Tensor grad_output, torch::Tensor input) {
    ...
    torch::Tensor grad_input_contig = grad_input.contiguous();
...
torch::Tensor linear_forward(torch::Tensor input, torch::Tensor weight, torch::Tensor bias) {
    ...
    torch::Tensor output = torch::empty({batch_size, output_size}, input.options());

    torch::Tensor input_contig = input.contiguous();
    torch::Tensor weight_contig = weight.contiguous();
    torch::Tensor bias_contig = bias.contiguous();
    torch::Tensor output_contig = output.contiguous();

    dim3 threads_per_block(256);
    dim3 number_of_blocks((output_size + threads_per_block.x - 1) / threads_per_block.x,
                          batch_size);
